...
class Produto {
  ...
  async createProduct() {
    ...
    if (this.validate(produto)) {
      ...
      document.getElementById('categProduto').value = ''
  ...
  // Promessa de retornar apenas quando o FileReader terminar de carregar a imagem, retornando a string Base64
  readImage(arquivo) {
    return new Promise((resolve, reject) => { // Transformando a operação assíncrona do FileReader em uma Promise (permitindo o uso do async await)
      const leitor = new FileReader()
      leitor.onload = () => resolve(leitor.result) // Retorna os dados do arquivo (o que acontece quando a leitura termina com sucesso)
      leitor.onerror = (erro) => reject(erro) // Define o que acontece quando a leitura termina com falhas: rejeição da Promise
      leitor.readAsDataURL(arquivo) // Inicia a leitura realmente, lendo os bytes do arquivo, retornando uma string no formato Data URL
    })
  }
  ...
  async readData() {
    ...
    const produtosArray = document.getElementById('categProduto').value

    // As categorias são recebidas através de uma string simples e são separadas por vírgula (",") e transformadas num array
    produto.categProduto = produtosArray.split(",")
    produto.quantProduto = Number(document.getElementById('quantProduto').value)

    //Lógica da imagem: se o tamanho de arquivos for diferente de 0, temos a atribuição de valor na espera da leitura do arquivo na
  ...
  update() {
    ...
    let idEdit = null // O botão 'salvar' não tem um id
    ...
    salvarAlteracoes.onclick = async (evt) => {
      ...
      const categNovaArray = document.getElementById('categProdutoNovas').value
      ...
      produtoId.categProduto = categNovaArray.split(",")


      if (nomeNovo.trim() !== "") produtoId.nomeProduto = nomeNovo; // Só altera quando não clicou em 'cancelar'
      if (descNova.trim() !== "") produtoId.descProduto = descNova; // Só altera quando não clicou em 'cancelar'
